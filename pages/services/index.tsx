      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        <section className="mb-8 overflow-hidden rounded-3xl bg-gray-900 text-white shadow-xl">
          <div className="grid gap-0 md:grid-cols-[1.1fr_0.9fr] md:items-center">
            ...
            <img src="/images/hvac-hero.svg" alt="HVAC tools and service equipment" className="h-64 w-full object-cover sm:h-72 md:h-full" />
          </div>
        </section>
        ...
        <section className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {(content.services || []).map((s:any, index:number)=> (
            <div key={s.slug} className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow">
              <img src={s.image || fallbackServiceImages[index % fallbackServiceImages.length]} alt={s.imageAlt || `${s.title} HVAC service`} className="h-44 w-full object-cover sm:h-48" />
              <div className="p-5">
