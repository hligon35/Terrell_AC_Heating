...
const fallbackSpecials = [
  {
    ...
    image: '/images/hvac-maintenance.svg',
  ...
  },
  {
    ...
    image: '/images/hvac-installation.svg',
        ...
        <section className="grid gap-6 md:grid-cols-2">
          {specials.map((s:any,i:number)=>(
            <div key={`${s.title}-${i}`} className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow">
              <img src={s.image || fallbackSpecials[i % fallbackSpecials.length].image} alt={s.imageAlt || s.title} className="h-52 w-full object-cover sm:h-56" />
